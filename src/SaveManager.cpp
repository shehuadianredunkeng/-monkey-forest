...
string resourceKey(ResourceType type) {
    switch (type) {
...
optional<ResourceType> parseResourceKey(const string& token) {
...
string skillKey(SkillType type) {
    switch (type) {
...
optional<SkillType> parseSkillKey(const string& token) {
...
bool validSnapshot(const PlayerSnapshot& snapshot) {
    return snapshot.health >= 0 && snapshot.health <= 100 &&
           ...
           snapshot.skills.at(SkillType::Combat) <= 5 &&
...
bool SaveManager::saveGame(const string& path, const GameContext& ctx) const {
    ofstream out(path);
    if (!out) {
        return false;
    }
    ...
    out << "room " << quoted(player.getCurrentRoomId()) << '\n';

    for (const Item& item : player.getInventory().getItems()) {
        out << "item " << quoted(item.getId()) << ' '
            << quoted(item.getName()) << ' ' << (item.isImportant() ? 1 : 0)
            << ' ' << item.getCount() << '\n';
    }
...
bool SaveManager::loadGame(const string& path, GameContext& ctx) const {
    ifstream in(path);
    if (!in) {
        return false;
    }
    ...
    while (in >> key) {
        ...
        if (key == "item") {
            ...
            int important = 0;
            int count = 0;
            if (!(in >> quoted(id) >> quoted(name) >> important >> count)) {
                return false;
            }
