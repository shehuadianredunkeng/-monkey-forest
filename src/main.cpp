...
bool npcIsHere(const GameContext& ctx, const std::string& npcId) {
    ...
    return containsId(room->getVisibleNPCIds(ctx), npcId);
}
...
std::string buildGuideText(const GameContext& ctx) {
    ...
    if (!targetRoom.empty()) {
        ...
        if (ctx.player.getCurrentRoomId() != targetRoom) {
            ...
            output << "你已经在目标地点，输入 investigate 继续主线。";
        }
    } else {
...
int main() {
    ...
    while (true) {
        ...
        if (commandIs(command, "look", "查看") || command == "l") {
            ...
            const std::vector<std::string> visibleNPCs =
                room == nullptr
                    ? std::vector<std::string>{}
                    : room->getVisibleNPCIds(ctx);

            if (visibleNPCs.empty()) {
                std::cout << "当前房间没有可询问任务的 NPC。\n";
            } else {
                for (const std::string& npcId : visibleNPCs) {
                    std::cout << npcId << "："
                            << npcs.getNPCQuest(npcId, ctx)
                            << "\n";
                }
            }
