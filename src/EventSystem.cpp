...
bool EventSystem::canTriggerEvent(const std::string& eventId,
    ...
    if (event->kind == EventKind::Random &&
        ...
        int completedRandomEvents = 0;
        for (const auto& [id, candidate] : events_) {
            (void)id;
            if (candidate.kind == EventKind::Random &&
                ctx.world.hasFlag(candidate.completionFlag)) {
                ++completedRandomEvents;
            }
        }
        if (completedRandomEvents >= 2) {
            return false;
        }
    }
...
ActionResult EventSystem::resolveChoice(const Event& event,
    ...
    if (event.eventId == "event_final_choice") {
        if (option == 1) {
            // 最终反击的硬条件与测试/结局规则一致：
            // 声望达到60、战斗技能达到2级即可进入赫兹战斗。
            // flag_route_resist_ready 仍可作为前期剧情准备标记，
            // 但不再重复阻断最终战斗入口。
            if (player.getReputation() < 60 ||
                player.getSkillLevel(SkillType::Combat) < 2) {
                return makeResult(false,
                                  "反击路线需要声望达到60、战斗技能达到2级。");
            }
    ...
    if (event.eventId == "event_injured_child") {
        if (option == 1) {
            if (!player.hasItem("item_herb")) {
                return makeResult(false, "背包中没有草药，无法处理豆豆的伤口。");
            }
        ...
        if (option == 2) {
            if (player.getStamina() < 12) {
                return makeResult(false, "带豆豆脱离危险需要体力至少12点。");
            }
            player.changeStamina(-12);
            player.changeHealth(-5);
            player.changeReputation(5);
