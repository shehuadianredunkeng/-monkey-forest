...
bool isBlankEventRequest(const string& eventId) {
...
const Event* EventSystem::findEvent(const string& eventId) const {
    const auto it = events_.find(eventId);
    return it == events_.end() ? nullptr : &it->second;
}

string EventSystem::findPendingEventId(const WorldState& world) const {
...
bool EventSystem::canTriggerEvent(const string& eventId,
    ...
    for (const string& flag : event->requiredFlags) {
...
ActionResult EventSystem::triggerEvent(const string& eventId,
                                       GameContext& ctx) {
    if (events_.empty()) {
        return makeResult(false, "事件系统尚未准备完成，请重新开始游戏。");
    }

    string resolvedId = eventId;
    if (resolvedId == kRandomRequestId) {
        const string pendingId = findPendingEventId(ctx.world);
        if (!pendingId.empty()) {
...
ActionResult EventSystem::chooseEventOption(const string& eventId,
                                            int option,
                                            GameContext& ctx) {
    if (events_.empty()) {
        return makeResult(false, "事件系统尚未准备完成，请重新开始游戏。");
...
string EventSystem::getCurrentObjective(const GameContext& ctx) const {
    if (events_.empty()) {
    ...
    string targetName = event->requiredRoomId;
...
string EventSystem::getStageIntroduction(int stage) const {
    return getStageIntroductionText(stage);
}

string EventSystem::getEndingText(const string& endingId) const {
...
ActionResult EventSystem::resolveChoice(const Event& event,
    ...
    if (event.eventId == "event_tree_trial") {
        ...
        if (player.getStamina() < 8) {
            return makeResult(false, "体力至少需要8点才能取得补给。");
        }
        player.changeStamina(-8);
    ...
    if (event.eventId == "event_winter_shortage") {
        ...
        if (option == 2) {
            if (player.getStamina() < 15) {
                return makeResult(false, "体力至少需要15点才能冒雪寻找补给。");
            }
    ...
    if (event.eventId == "event_drought_choice") {
        if (option == 1) {
            if (player.getSkillLevel(SkillType::Combat) < 2) {
                return makeResult(false,
                                  "至少需要2级战斗技能才能组织守卫训练。");
            }
        ...
        if (player.getSkillLevel(SkillType::Leadership) < 2 ||
            player.getStamina() < 10) {
            return makeResult(false, "寻找新家园需要领导技能≥2且体力≥10。");
        }
        player.changeStamina(-10);
        player.changeReputation(5);
    ...
    if (event.eventId == "event_group_dispute") {
        ...
        if (option == 1) {
            if (player.getSkillLevel(SkillType::Leadership) < 2 &&
                ...
                return makeResult(false,
                                  "调解需要领导技能至少2级或声望至少20点。");
            }
            player.changeReputation(10);
            world.changeResource(ResourceType::Morale, 10);
            if (!world.hasFlag("flag_scout_left")) {
    ...
    if (event.eventId == "event_base_infiltration") {
        ...
        if (option == 2) {
            ...
            return completeEvent(
                event,
                "晶片骗过了控制台，你下载了完整日志。智慧+1，智取路线准备完成。",
                ctx);
        }
        if (!world.hasFlag("flag_scout_help") ||
            world.hasFlag("flag_scout_left")) {
            return makeResult(false,
                              "闪尾当前不在队伍中，无法选择由他引开巡逻机。");
        }
        player.changeReputation(8);
        world.setFlag("flag_complete_log");
        return completeEvent(
    ...
    if (event.eventId == "event_final_choice") {
        if (world.hasFlag("flag_escape_normal_endings_locked")) {
            world.setFlag("flag_bad_ending_coward");
            world.setFlag("flag_final_choice");
            return completeEvent(
                event,
                "你站在最终抉择前，却发现每条路都少了曾经被你逃避的那一步。"
                "累计五次逃离战斗，三条常规路线已全部关闭。"
                "\n【结局达成】你是狗熊",
                ctx);
        }
        if (option == 1) {
