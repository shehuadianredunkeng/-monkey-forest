...
Code nextCode(const wstring& text, size_t& index) {
...
vector<wstring> splitGlyphs(const wstring& text) {
    ...
    for (size_t i = 0; i < text.size();) {
        ...
        if (code == L'\t') code = L' '; // \t 换成空格，输出里不留控制字符
...
int portableColumns(const wstring& glyph) {
    // 没有控制台时的兜底宽度；Windows 下走实际光标位移测量。
    int cells = 0;
...
wstring clipText(const wstring& text, int columns, const Measure& measure) {
    ...
    for (const auto& glyph : splitGlyphs(text)) {
        if (glyph == L"\n") break;
        const int width = max(0, measure(glyph));
...
vector<wstring> wrapText(const wstring& text, int columns, const Measure& measure) {
    ...
    for (const auto& glyph : splitGlyphs(text)) {
        ...
        int width = max(0, measure(glyph));
        const wstring shown = width > columns ? L"?" : glyph;
        if (width > columns) width = max(1, measure(shown));
...
wstring fromUtf8(const string& text) {
    ...
    for (size_t i = 0; i < text.size();) {
        ...
        if (i - start != static_cast<size_t>(count) || code < minimum[count]) code = 0xfffd;
        result += wide(code);
    }
    return result;
}
