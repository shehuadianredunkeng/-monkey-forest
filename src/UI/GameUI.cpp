...
wstring join(const vector<wstring>& glyphs, size_t from, size_t to) {
    wstring text;
    for (auto i = from; i < to; ++i) text += glyphs[i];
    return text;
}
Color logColor(const wstring& text) {
    if (text.find(L"【智慧 +1】") != wstring::npos || text.find(L"智慧+1") != wstring::npos ||
        text.find(L"成功") != wstring::npos) return Color::Success;
    if (text.find(L"失败") != wstring::npos || text.find(L"无法") != wstring::npos ||
        text.find(L"不足") != wstring::npos || text.find(L"错误") != wstring::npos) return Color::Error;
    if (text.find(L"> ") == 0 || text.find(L"提示") != wstring::npos) return Color::Hint;
    return Color::Normal;
}
}
...
GameView readGameView(const GameContext& ctx, bool inBattle,
    ...
    wstring clean = objectiveText;
    // 目标文本可能包含内部ID，只在显示层隐藏，不修改剧情状态。
    for (auto start = clean.find(L"（room_"); start != wstring::npos; start = clean.find(L"（room_")) {
        const auto end = clean.find(L'）', start);
        if (end == wstring::npos) { clean.erase(start); break; }
        clean.erase(start, end - start + 1);
    }
    const auto newline = clean.find(L'\n');
    view.taskTitle = clean.substr(0, newline);
    const wstring prefix = L"当前主线：";
    if (view.taskTitle.find(prefix) == 0) view.taskTitle.erase(0, prefix.size());
    view.taskHint = newline == wstring::npos
                        ? L"请根据右侧目标前往彩色任务点"
                        : clean.substr(newline + 1);
    view.taskStatus = player.getHealth() <= 0 || world.hasFlag("flag_final_choice") ? L"已结束" :
        clean.find(L"已完成") != wstring::npos || clean.find(L"已经完成") != wstring::npos ? L"已完成" : L"进行中";
    return view;
}

GameUI::GameUI(ConsoleRenderer& renderer) : renderer_(renderer) {}
void GameUI::appendLog(const wstring& text) {
    if (text.empty()) return;
    // 存没折行的逻辑行，换字体后按新宽度重新排。
    size_t from = 0;
    while (from < text.size()) {
        const auto end = text.find(L'\n', from);
        const auto line = text.substr(from, end == wstring::npos ? end : end - from);
...
void GameUI::paragraph(Rect area, const wstring& text, Color color) {
    const int width = area.right - area.left + 1;
    const auto lines = renderer_.wrapText(text, width);
    const auto count = min<size_t>(lines.size(), area.bottom - area.top + 1);
...
void GameUI::bar(SHORT y, int value) {
    ...
    renderer_.drawText(RIGHT + 12, y, to_wstring(value) + L"/100");
}
bool GameUI::render(const GameView& view) {
    ...
    constexpr size_t rows = LEFT.bottom - LEFT.top + 1;
    scrollBack_ = min(scrollBack_, wrapped.size() > rows ? wrapped.size() - rows : 0);
    ...
    renderer_.drawText(RIGHT, 15, L"智慧 WIS: " + to_wstring(view.wisdom));
    renderer_.drawText(RIGHT, 16, L"力量 STR: " + to_wstring(view.strength));
    renderer_.drawText(RIGHT, 17, L"声望: " + to_wstring(view.reputation));
    renderer_.drawText(RIGHT, 18, L"背包格数: " + to_wstring(view.inventorySlots) + L"/12");
    ...
    renderer_.drawFrame(); // 必须最后画：文字位置不能反过来决定边框坐标
    return true;
}
void GameUI::drawInput(const vector<wstring>& glyphs, size_t caret) {
    constexpr int available = UI_WIDTH - 4; // 留一格给行尾光标
    size_t start = caret;
    int before = 0;
    while (start > 0) {
...
optional<wstring> GameUI::readCommand() {
    ...
    while (true) {
        ...
        switch (event.key) {
            case Key::EndOfInput: return nullopt;
            case Key::Enter: {
            ...
            case Key::Text: {
                ...
                prefix.erase(remove(prefix.begin(), prefix.end(), L"\n"), prefix.end());
                const auto tail = input.size() - caret;
                if (prefix.size() + tail > 1024) break;
                prefix.insert(prefix.end(), input.begin() + static_cast<ptrdiff_t>(caret), input.end());
                caret = prefix.size() - tail;
                input = move(prefix);
                break;
            }
            case Key::Backspace:
                if (caret > 0) { input.erase(input.begin() + static_cast<ptrdiff_t>(caret - 1)); --caret; }
                break;
            case Key::Delete:
                if (caret < input.size()) input.erase(input.begin() + static_cast<ptrdiff_t>(caret));
                break;
