...
namespace {
using PointPair = pair<int, int>;

vector<string> terrain(int width, int height) {
    vector<string> result(static_cast<size_t>(height),
...
void scatter(vector<string>& map, char tile,
             initializer_list<PointPair> points) {
    for (const auto& point : points) map[point.second][point.first] = tile;
}

void horizontal(vector<string>& map, int y, int from, int to,
                char tile = '#') {
    for (int x = from; x <= to; ++x) map[y][x] = tile;
}

void vertical(vector<string>& map, int x, int from, int to,
              char tile = '#') {
    for (int y = from; y <= to; ++y) map[y][x] = tile;
}

vector<string> treeTerrain(int width, int height) {
...
vector<string> forestTerrain(int width, int height) {
    auto map = terrain(width, height);
...
vector<string> riverTerrain(int width, int height) {
    auto map = terrain(width, height);
...
vector<string> caveTerrain(int width, int height) {
    auto map = terrain(width, height);
...
vector<string> baseTerrain(int width, int height) {
    auto map = terrain(width, height);
    // 金属隔墙划分实验区、控制区与中央走廊。
    vertical(map, 9, 2, 5);
    vertical(map, 9, 9, 12);
...
string itemTakenFlag(const string& roomId, const string& itemId) {
    return "flag_taken_" + roomId + "_" + itemId;
}

string npcName(const string& id) {
...
string itemName(const string& id) {
...
string enemyName(const string& id) {
...
void InteractiveMap::resetForRoom(const GameContext& ctx,
    ...
    if (enteredByDirection.empty()) {
        ...
        for (const string& flag : ctx.world.getFlags()) {
            ...
            istringstream encoded(flag.substr(prefix.size()));
            int x = 0;
            int y = 0;
            char separator = 0;
...
void InteractiveMap::storePosition(GameContext& ctx) const {
    ...
    ctx.world.setFlag("flag_map_position_" + roomId_ + "_" +
                      to_string(player_.x) + "_" +
                      to_string(player_.y));
}

InteractiveMap::Point InteractiveMap::spawnAfterTransition(
    const Definition& target, const string& direction) const {
    if (direction == "east") {
...
bool InteractiveMap::npcVisible(const string& npcId,
    ...
    if (npcId == "npc_child")
        return childReturnedToTree(ctx.world) ? roomId_ == "room_tree"
                                             : roomId_ == "room_river";
    return true;
}

bool InteractiveMap::itemVisible(const string& itemId,
                                 const GameContext& ctx) const {
    return !ctx.world.hasFlag(itemTakenFlag(roomId_, itemId));
}

bool InteractiveMap::enemyVisible(const string& enemyId,
                                  const GameContext& ctx) const {
    if (enemyId.find('@') != string::npos)
...
bool InteractiveMap::doorLocked(const string& direction,
                                const GameContext& ctx) const {
...
vector<InteractiveMap::Point> InteractiveMap::dynamicPoints(
    const GameContext& ctx) const {
    const Definition* map = current();
    if (map == nullptr) return {};
    ...
    if (!available.empty()) {
        ...
        rotate(available.begin(), available.begin() + rotation,
                    available.end());
    }
    return available;
}
...
MapInteraction InteractiveMap::companionAt(Point point,
    ...
    const vector<Point> points = dynamicPoints(ctx);
...
MapInteraction InteractiveMap::seasonalGuardianAt(
    ...
    const int season = seasonIndex(ctx.world.getTurnCount());
    if (roomId_ != seasonRoom(season) ||
        ctx.world.hasFlag(string("flag_season_relic_") +
                          seasonId(season))) return {};
    const vector<Point> points = dynamicPoints(ctx);
    if (points.size() < 3) return {};
    const string defeated = string("flag_season_guardian_defeated_") +
                                 seasonId(season);
    if (point == points[2] && ctx.world.hasFlag(defeated))
        return {InteractionKind::EasterEgg,
                string("season_relic_") + seasonId(season),
                string("拾取稀有信物·") + seasonChinese(season)};
    if (point == points[2])
        return {InteractionKind::LockedItem,
                string("season_relic_") + seasonId(season),
                string("稀有信物·") + seasonChinese(season) +
                    "被守望者保护着，需先挑战旁边的红色敌人"};
    if (point == points[1])
        return {InteractionKind::Enemy,
                string("enemy_season_guardian_") + seasonId(season) +
                    "@" + roomId_ + "@" +
                    to_string(ctx.world.getTurnCount() / 6),
                string("挑战守护") + seasonChinese(season) + "的四季守望者"};
    return {};
}
...
MapInteraction InteractiveMap::randomEventAt(Point point,
    ...
    const string id = "location_event_" + roomId_ + "_" +
                           to_string(ctx.world.getTurnCount());
...
MapInteraction InteractiveMap::dynamicEnemyAt(Point point,
                                              const GameContext& ctx) const {
    const vector<Point> points = dynamicPoints(ctx);
    for (size_t slot = 4; slot < points.size() && slot < 6; ++slot) {
        if (!(point == points[slot])) continue;
        const bool mechanical = roomId_ == "room_cave" || roomId_ == "room_base";
        const string base = mechanical ? "enemy_drone" : "enemy_raider";
        const string id = base + "@" + roomId_ + "@" +
            to_string(ctx.world.getTurnCount()) + "@" +
            to_string(slot - 4);
...
MapTileVisual InteractiveMap::visualAt(int x, int y,
    ...
    const auto npc = map->npcs.find(point);
    if (npc != map->npcs.end() && npcVisible(npc->second, ctx)) {
        static const std::map<string, wstring> initials{
            {"npc_king", L"岩"}, {"npc_healer", L"叶"},
            {"npc_scout", L"闪"}, {"npc_child", L"豆"}, {"npc_hertz", L"赫"}};
        const auto name = initials.find(npc->second);
        return {name != initials.end() ? name->second : L"人", UI::Color::Npc};
    }
    const auto item = map->items.find(point);
    if (item != map->items.end() && itemVisible(item->second, ctx))
...
const string& InteractiveMap::roomId() const { return roomId_; }
