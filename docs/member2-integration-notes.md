...
- 每次选择后显示结果和下一步，不向玩家显示内部ID、程序调用名或已删除的 `guide` 指令。
- 野蜂、巡逻机、赫兹战斗胜利后可由主循环调用2号接口自动续接剧情。
- 5个主线智慧成长节点可随存档保存；随机事件不再增加智慧或力量。
- 2号事件中的攀爬技能判定已改为力量和体力判定，不影响1号地图自己的移动规则。
...
| 选项流程文字清晰 | 结果说明具体变化；完成后提示查看界面主线目标；战斗后提供自动续接接口 |
| 丰富主线和结局 | 六阶段和11个事件均重新润色；四个结局逐行排版，并显示条件和评价语 |
| 攀爬判定改为力量 | 树冠试炼第二项与回声追踪第二项只检查力量和体力 |
| 奇遇不增加核心属性 | 侦察机奇遇取消智慧奖励，改为声望奖励；智慧成长全部放回主线 |
...
不要在 `main.cpp` 另写一套事件ID、目标房间或英文任务名。根据最新测试意见，
命令列表不再需要提供 `guide` / `指引`；2号事件结果也不会提示玩家输入该命令。
...
| 星猿技术 | 干旱抉择中的晶片研究 | 智慧≥2后智慧+1，并完成智取路线准备 |
| 基地日志 | 基地潜入第二项 | 智慧达到3后+1，可补到最终要求4 |

玩家初始智慧为1，即使错过树冠观察，仍可依次通过管线规律、回声碑文和晶片研究
达到智慧4。侦察机坠落属于随机奇遇，只奖励晶片和声望，不再增加智慧或力量。
所有主线奖励依靠事件完成状态防重复。

## 7. 结局文本与显示接口

2号继续提供4个现有结局的完整文本，不改变5号已有的结局ID：

| 结局ID | 中文结局 | 文本中显示的条件 |
|---|---|---|
| `ending_resist` | 青木英雄 | 反击准备、声望、战斗技能和赫兹胜利 |
| `ending_hack` | 无声胜利 | 技术研究、智慧和完整日志 |
| `ending_migrate` | 向南的新生 | 迁徙准备、物资、领导技能和新家园 |
| `ending_fail` | 失落之谷 | 生命耗尽或未完成有效最终选择 |

每份文本均包含方括号条件、逐行剧情和方括号评价语。4号可以按换行符逐行显示，
并在UI层实现全屏、每行1秒和结尾等待；2号不控制终端清屏、颜色或计时。

“增加新结局”不能由2号单独完成：如果团队确定新的结局ID与判定旗标，2号再补写
对应文本，5号负责判定，4号负责显示。当前版本保持原4个ID，避免破坏现有联调。

3号成就系统可以直接使用现有事件完成旗标建立更多成就，例如完成全部随机事件、
不战斗取得日志、三条路线准备等；2号不重复实现成就存储和解锁界面。
...
2号负责的文件是：`EventSystem.h`、`EventFactory.cpp`、`EventSystem.cpp`、
`StoryText.cpp` 和2号测试。地图、NPC、战斗、玩家、背包、主循环、存档、状态显示和
结局判定分别由其他成员负责。

## 11. 验收测试

2号专项测试在 `tests/member2_event_test.cpp`，覆盖主线与随机事件的触发、选项条件、
防重复结算、战斗续接、三条结局路线的可达性，以及读档后的待选事件恢复。

整个工程的地图、2号、3号、4号、5号测试均通过。
