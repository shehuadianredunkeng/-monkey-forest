...
void testStatusViewContainsCoreState() {
    ...
    const string status = buildStatusText(ctx);
    expect(status.find("阶段：3") != string::npos,
           "status should show stage");
    expect(status.find("回合：7") != string::npos,
           "status should show turns");
    expect(status.find("清泉河谷") != string::npos,
           "status should show current room name");
    expect(status.find("生命：90") != string::npos,
           "status should show health");
    expect(status.find("体力：55") != string::npos,
           "status should show stamina");
    expect(status.find("声望：12") != string::npos,
           "status should show reputation");
    expect(status.find("食物2") != string::npos,
           "status should show food");
    expect(status.find("水源4") != string::npos,
           "status should show water");
    expect(status.find("领导2") != string::npos,
           "status should show leadership skill");
    expect(status.find("采集") == string::npos,
           "removed gathering skill must not appear in status");
    expect(status.find("已找到星猿晶片") != string::npos,
           "status should show readable key flags");
    expect(status.find("flag_chip_found") == string::npos &&
               status.find("room_river") == string::npos,
           "status should hide internal ids");
}
...
int main() {
    try {
        ...
        cerr << "member5_state_save_test failed: " << error.what() << '\n';
        return 1;
    }
    cout << "member5_state_save_test passed\n";
    return 0;
}
