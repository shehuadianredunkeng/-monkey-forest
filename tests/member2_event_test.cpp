...
void expectPlayerFacingStory(const ActionResult& result,
    ...
    expect(result.message.find("startBattle") == string::npos &&
               result.message.find("enemy_") == string::npos &&
               result.message.find("choose 1") == string::npos,
...
void testAllElevenEventsCanTrigger() {
    ...
    for (const TriggerCase& testCase : cases) {
        ...
        for (const string& flag : testCase.flags) {
            world.setFlag(flag);
        }
...
void testEveryChoiceShowsItsRequirements() {
    ...
    for (const auto& [id, event] : events) {
        for (const string& choice : event.choices) {
            expect(choice.find("需要") != string::npos ||
                       choice.find("无属性要求") != string::npos ||
                       choice.find("无智慧门槛") != string::npos,
...
void testRoomEntryAutoTriggerAndChineseObjective() {
    ...
    const string beforeMove = events.getCurrentObjective(ctx);
    expect(beforeMove.find("树冠试炼") != string::npos &&
               beforeMove.find("果实森林") != string::npos,
           "objective should show the Chinese event and room names");
    expect(beforeMove.find("event_") == string::npos &&
               beforeMove.find("room_") == string::npos,
           "objective must hide internal identifiers");

    player.setCurrentRoomId("room_forest");
    const string atForest = events.getCurrentObjective(ctx);
    expect(atForest.find("随机（random）") != string::npos,
    ...
    expect(prompt.success &&
               prompt.message.find("【事件】树冠试炼") != string::npos,
...
void testMainEventAndDuplicateProtection() {
    ...
    expect(prompt.message.find("【事件】树冠试炼") != string::npos &&
               prompt.message.find("需要力量≥2且体力≥10") != string::npos,
           "main prompt should use Chinese and show option requirements");
    expect(prompt.message.find("event_tree_trial") == string::npos &&
               prompt.message.find("flag_") == string::npos,
...
void testEventClimbChecksAreReplacedByStrength() {
    ...
    for (const auto& [id, event] : allEvents) {
        for (const string& choice : event.choices) {
            expect(choice.find("攀爬") == string::npos,
...
void testAllRandomEventsCanBeCompleted() {
    ...
    expect(firstResult.message.find("指引") == string::npos &&
               firstResult.message.find("guide") == string::npos &&
               firstResult.message.find("右侧当前目标") != string::npos,
...
void testAllRoutesAchievementHasAPlayablePath() {
    Player player;
    WorldState world;
    resetWorld(world);
    auto rooms = createAllRooms();
    ...
    player.setCurrentRoomId("room_river");
    expect(events.triggerEvent("event_drought_choice", ctx).success,
           "migration preparation event should trigger");
    expect(events.chooseEventOption("", 3, ctx).success &&
               world.hasFlag("flag_route_migrate_ready"),
           "migration preparation should be obtainable first");

    player.setCurrentRoomId("room_tree");
    expect(events.triggerEvent("event_group_dispute", ctx).success,
           "group dispute should trigger after drought");
    expect(events.chooseEventOption("", 2, ctx).success &&
               world.hasFlag("flag_route_resist_ready"),
           "resist preparation should be obtainable second");
    ...
    expect(events.chooseEventOption("", 2, ctx).success &&
           ...
           "technical infiltration should complete hack preparation third");
    ...
    expect(collections.isAchievementUnlocked("achievement_all_routes_ready", world),
           "three-routes achievement must unlock through normal gameplay");
}

void testFiveEscapesOverrideNormalFinalRoutes() {
    Player player;
    WorldState world;
    resetWorld(world);
    auto rooms = createAllRooms();
    ...
    world.setFlag("flag_complete_log");
    world.setFlag("flag_route_hack_ready");
    world.setFlag("flag_escape_normal_endings_locked");
    player.setCurrentRoomId("room_tree");
    player.changeWisdom(3);
    expect(events.triggerEvent("event_final_choice", ctx).success,
           "final choice should still open after five escapes");
    const ActionResult ending = events.chooseEventOption("", 2, ctx);
    expect(ending.success && ending.stageCompleted &&
               world.hasFlag("flag_bad_ending_coward") &&
               !world.hasFlag("flag_choice_hack"),
...
void testPendingRecoveryAndStoryText() {
    ...
    expect(loaded.getStageIntroduction(1).find("第一年") != string::npos,
    ...
    const string endingIds[] = {
        "ending_resist", "ending_hack", "ending_migrate", "ending_fail"};
    for (const string& id : endingIds) {
        const string ending = loaded.getEndingText(id);
        expect((ending.find("[解锁条件：") != string::npos ||
                ending.find("[触发条件：") != string::npos) &&
                   ending.find("[结局评价：") != string::npos,
               "ending should show its condition and evaluation: " + id);
    }
    for (int stage = 1; stage <= 6; ++stage) {
        const string intro = loaded.getStageIntroduction(stage);
        expect(intro.find("event_") == string::npos &&
                   intro.find("flag_") == string::npos,
...
int main() {
    try {
        ...
        testFiveEscapesOverrideNormalFinalRoutes();
        testAllThreeEndingRoutesAreReachableAndPrepared();
        testPendingRecoveryAndStoryText();
    } catch (const exception& error) {
        cerr << "member2_event_test failed: " << error.what() << '\n';
        return 1;
    }
    cout << "member2_event_test passed\n";
    return 0;
}
