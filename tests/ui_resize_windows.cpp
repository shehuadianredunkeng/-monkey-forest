...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (GetCurrentConsoleFontEx(out, FALSE, &font)) {
            font.dwFontSize = {8,16}; SetCurrentConsoleFontEx(out,FALSE,&font);
        }
        ...
        for (auto size : {COORD{111,34},COORD{160,42},COORD{123,36},COORD{111,30},COORD{111,34}}) {
            ...
            for (SHORT y : {SHORT(0),SHORT(size.Y-5),SHORT(size.Y-1)}) {
                for (SHORT x=0;x<size.X;++x) {
                    wchar_t ch=0; DWORD n=0;
                    expect(ReadConsoleOutputCharacterW(out,&ch,1,{x,y},&n) && n==1,"read border");
                    wchar_t expected=(x==0 || x==size.X-1 || (x==UI::DIVIDER_X && y!=size.Y-1))?L'+':L'-';
                    expect(ch==expected,"horizontal border missing or overwritten");
                }
            }
            for (SHORT y=1;y<size.Y-1;++y) {
                wchar_t ch=0; DWORD n=0;
                expect(ReadConsoleOutputCharacterW(out,&ch,1,{SHORT(size.X-1),y},&n),"read right edge");
                expect(ch==((y==8 || y==20 || y==size.Y-5)?L'+':L'|'),"right border missing");
            }
            CONSOLE_SCREEN_BUFFER_INFO info{};
            expect(GetConsoleScreenBufferInfo(out,&info),"read viewport");
            expect(info.srWindow.Top==0 && info.srWindow.Bottom==size.Y-1,"render scrolled viewport");
            report << size.X << "x" << size.Y << ": complete borders, no scrolling\n";
        }
        report << "PASS\n";
        return 0;
    } catch (const exception& e) { report << "FAIL: " << e.what() << '\n'; return 1; }
}
